  ...
  useEffect(() => {
    ...
    loader.load(
      ...
      (gltf) => {
        ...
        const card = document.querySelector('.service-card:nth-child(10)');
        if (card) {
          card.addEventListener('mouseenter', () => {
            gsap.to(model.rotation, { y: model.rotation.y + Math.PI, duration: 0.7, ease: 'power2.inOut' });
            if (!hasEntered) {
      ...
      (err) => console.warn('Caliper failed:', err)
    );
    ...
    const animate = () => {
      ...
      if (modelRef) {
        ...
        modelRef.position.y = Math.sin(caliperTime * 1.3) * 0.04;
      }
