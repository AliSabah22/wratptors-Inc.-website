...
import { detailingBrushTransforms, modelTargetDisplaySizes, getBreakpoint } from '@/lib/transformConfigs';

export default function DetailingBrushCanvas() {
  ...
  useEffect(() => {
    ...
    loader.load(
      ...
      (gltf) => {
        ...
        const targetDisplaySize = modelTargetDisplaySizes.detailingBrush;
        applyModelCorrection(model, { targetDisplaySize });
        recenterAfterTransform(model);

        const layout = detailingBrushTransforms[getBreakpoint(window.innerWidth)];
      ...
      undefined,
      (err) => console.warn('Detailing brush failed to load:', err)
    );
  ...
  return <div ref={containerRef} id="detailing-brush-canvas" className="service-model-canvas" />;
}
